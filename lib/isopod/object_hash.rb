...
class Isopod
  ...
  class ObjectHash
    ...
    def initialize(input, opts = {})
      ...
      @xref        = Isopod::XRef.new(@io)
      @pdf_version = read_version
      @trailer     = @xref.trailer
      @cache       = opts[:cache] || Isopod::ObjectCache.new
    ...
    def stream?(ref)
      self.has_key?(ref) && self[ref].is_a?(Isopod::Stream)
    end
    ...
    def [](key)
      ...
      unless key.is_a?(Isopod::Reference)
        key = Isopod::Reference.new(key.to_i, 0)
      end
      ...
      elsif xref[key].is_a?(Isopod::Reference)
    ...
    #
    def object(key)
      key.is_a?(Isopod::Reference) ? self[key] : key
    end
    alias :deref :object
